...
const DeleteOrganizationButton = () => {
    ...
    return (
        <div>
            <button className={'add-admin-button'} style={{color:'white',backgroundColor:'#F33F19'}} onClick={handleOpen}>Supprimer</button>
            <Modal
                open={open}
                onClose={handleClose}
            ...
            >
                <Box sx={{ ...style, borderRadius: '8px', textAlign: 'center', padding:'48px 0', position:'relative'}}>
                    ...
                    <p id="parent-modal-description" className={'dialogContent'}>
                        Voulez-vous supprimer l’organisation <span className={'fw-bold'}>Organisation</span> et tout son contenu ?
                    </p>
